...
public class CommandHunger {
    ...
    public static void register(CommandDispatcher<ServerCommandSource> serverCommandSourceCommandDispatcher,
        ...
        for(String CommandLine: NAMESPACE) {
            serverCommandSourceCommandDispatcher.register(CommandManager.literal(CommandLine+":hunger")
                    .requires(source -> source.hasPermissionLevel(2))
                    .executes(CommandHunger::run)
                    .then(CommandManager.argument("targets", EntityArgumentType.entities())
                            .executes(context -> execute((ServerCommandSource)context.getSource(), EntityArgumentType.getPlayers(context, "targets"))))
    ...
    public static int execute(ServerCommandSource source, Collection<ServerPlayerEntity> targets) {
        for (ServerPlayerEntity serverPlayerEntity : targets) {
            serverPlayerEntity.getHungerManager().setFoodLevel(0);
            serverPlayerEntity.getHungerManager().setSaturationLevel(0f);

            serverPlayerEntity.sendMessage(Text.translatable("commands.hunger.done"), false);
        }
        return targets.size();
    }
