...
import static com.KafuuChino0722.coreextensions.CoreManager.TAG_PRESSURE_PLATES_BLOCK;
import static com.KafuuChino0722.coreextensions.CoreManager.TAG_PRESSURE_PLATES_ITEM;
...
public class PressurePlate {
    public static void register(String name, String namespace, String id, int maxCount, Map<String, Object> blockData, Map<String, Object> properties, Boolean dropsNothing, BlockSoundGroup customSound ,double hardness, double resistance, boolean generate){
        boolean SetType = properties.containsKey("stone") ? (boolean) properties.get("stone") : false;
        ...
        PressurePlateBlock.ActivationRule PPBAR = PressurePlateBlock.ActivationRule.EVERYTHING;
        ...
        Block block = new PressurePlateBlock(PPBAR, blockSettings, blockType);
        try {
        ...
        if(generate) {
            Models.generate(namespace, id, "PRESSUREPLATE");
        }

        String identifier = namespace + ":" +id;
        //Loots.addDrop(namespace, id, blockData, properties);
        Loots.addDropPath(namespace, id, "blocks/", blockData, properties);
