...
public class iZipGameRule {
    ...
    public static void load(String[] paths) {
        for (String path : paths) { // 迭代每个路径
            loadGameRulesFromZipFiles(new File(path));
        }
    }
        ...
        if (gameRulesData != null && gameRulesData.containsKey("gamerules")) {
            ...
            for (Map.Entry<String, Object> entry : gamerules.entrySet()) {
                if (entry.getValue() instanceof Boolean) {
                    ...
                    if(!GameRuleRegistry.hasRegistration(ruleName)) {
                        ...
                        Info.custom("Gamerule " + gameRuleKey + defaultValue + " registered!","Gamerules");
                    }
                }
            }
        }
