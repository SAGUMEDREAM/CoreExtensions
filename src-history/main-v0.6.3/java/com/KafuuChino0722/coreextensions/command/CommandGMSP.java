...
public class CommandGMSP {
    public static void register(CommandDispatcher<ServerCommandSource> serverCommandSourceCommandDispatcher,
                                CommandRegistryAccess commandRegistryAccess,
                                CommandManager.RegistrationEnvironment registrationEnvironment) {
        serverCommandSourceCommandDispatcher.register(CommandManager.literal("gmsp")
                .requires(source -> source.hasPermissionLevel(2))
                .executes(CommandGMSP::run)
        ...
        for(String CommandLine: NAMESPACE) {
            serverCommandSourceCommandDispatcher.register(CommandManager.literal(CommandLine+":gmsp")
                    .requires(source -> source.hasPermissionLevel(2))
                    .executes(CommandGMSP::run)
                    .then(CommandManager.argument("targets", EntityArgumentType.entities())
                            .executes(context -> execute((ServerCommandSource)context.getSource(), EntityArgumentType.getPlayers(context, "targets")))));
    ...
    public static int execute(ServerCommandSource source, Collection<ServerPlayerEntity> targets) {
        for (ServerPlayerEntity serverPlayerEntity : targets) {
            serverPlayerEntity.sendMessage(Text.translatable("commands.gm.3"), false);
    ...
    public static int run(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
        ...
        player.sendMessage(Text.translatable("commands.gm.3"), false);
        GameMode newGameMode = GameMode.SPECTATOR;
