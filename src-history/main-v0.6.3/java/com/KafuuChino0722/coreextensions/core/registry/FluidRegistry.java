        ...
        if (itemsData != null && itemsData.containsKey("blocks")) {
            ...
            for (Map.Entry<String, Object> entry : items.entrySet()) {
                if (entry.getValue() instanceof Map) {
                    ...
                    if(Registries.FLUID.containsId(new Identifier(namespace,id))) {
                        StorageRegistry.add(StorageRegistry.BLOCKS,new Identifier(namespace,id));
                        StorageRegistry.add(StorageRegistry.FLUIDS,new Identifier(namespace,id));
                        StorageRegistry.add(StorageRegistry.FLUIDS,new Identifier(namespace,"flowing_" + id));
                        StorageRegistry.add(StorageRegistry.ITEMS,new Identifier(namespace,id));
                        StorageRegistry.add(StorageRegistry.ITEMS,new Identifier(namespace,id + "_bucket"));
                    }
                }
            }
        }
