...
public class EventOnUseAbstractBlockRegistry implements EventInterface {
    public static void register(String namespace, String id, BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        Map<String, Map<String, Object>> itemsData = IOFileManager.read(Events.EVENT);
        load(namespace, id, itemsData, state, world, pos, player, hand, hit);
        Map<String, Map<String, Object>> itemsDataZ = IOFileManager.readZip(Events.EVENT);
        load(namespace, id, itemsDataZ, state, world, pos, player, hand, hit);
    }
    ...
    public static void load(String namespace, String id, Map<String, Map<String, Object>> eventsData, BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (eventsData != null && eventsData.containsKey("block_events")) {
            ...
            for (Map.Entry<String, Object> entry : events.entrySet()) {
                if (entry.getValue() instanceof Map) {
                    ...
                    ItemUsageContext context = new ItemPlacementContext(player,hand,player.getActiveItem(),hit);
                    ...
                    if (applyKey != null) {
                        for (String apply : applyKey) {
                            if (apply.equalsIgnoreCase(ItemIdentifier)) {
                                ...
                                if (actionsKey != null) {
                                    Map<String, Object> Key = (Map<String, Object>) actionsKey.getOrDefault("right_block", null);
                                    if (Key != null) {
                                        ...
                                        if(condition!=null) {
                                            ...
                                            if(blockKey!=null) {
                                                fail = Conditions.getCheckBlock(state, world, pos, player, hand, hit, blockKey);
                                                if(fail) return;
                                            }
