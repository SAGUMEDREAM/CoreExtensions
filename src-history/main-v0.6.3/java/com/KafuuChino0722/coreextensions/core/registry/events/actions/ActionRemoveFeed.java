...
public class ActionRemoveFeed implements ActionInterface {
    public static void run(World world, LivingEntity user, Hand hand, Map<String, Object> Key) {
        Map<String, Object> HKey = (Map<String, Object>) Key.getOrDefault("removeFeed", null);
        if(HKey!=null) {
            ...
            if (HKey != null && !player.getAbilities().creativeMode) {
                ...
                hungerM.setFoodLevel(hungerM.getFoodLevel() - (int) hunger);
                hungerM.setSaturationLevel(hungerM.getSaturationLevel() - (float) saturationModifier);
            }
        }
    }
}
