...
public class ItemWeaponsRegistry {
    public static final String FILE = Reference.File;

    public static void register() {
        Map<String, Map<String, Object>> itemsData = IOFileManager.read("itemWeapon.yml");
        load(itemsData);
        Map<String, Map<String, Object>> itemsDataZ = IOFileManager.readZip("itemWeapon.yml");
        load(itemsDataZ);
    }
        ...
        if (itemsData != null && itemsData.containsKey("items")) {
            ...
            for (Map.Entry<String, Object> entry : items.entrySet()) {
                if (entry.getValue() instanceof Map) {
                    ...
                    int maxCount = (int) itemData.getOrDefault("maxCount",1);
                    ...
                    switch (types.toLowerCase()) {
                        ...
                        case "crossbow" -> {
                            ItemSettings = ItemSettings.maxDamage(durability);
                            ItemType = new CrossbowItem(ItemSettings) {
                                @Override
                                public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
                                    if (tooltipMsg != null) {
                                        for (String Line : tooltipMsg) {
