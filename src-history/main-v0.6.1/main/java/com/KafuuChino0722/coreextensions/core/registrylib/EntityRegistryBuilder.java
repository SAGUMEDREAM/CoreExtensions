...
public class EntityRegistryBuilder<E extends Entity> {
    ...
    public EntityType<E> build() {
        ...
        EntityType<E> entityType = Registry.register(Registries.ENTITY_TYPE, IdentifierVoid, entityBuilder.build(IdentifierVoid.getPath()));

        if (this.hasEgg) {
            Item spawn_egg  = com.KafuuChino0722.coreextensions.core.registrylib.Registry.registerEgg(
                    new SpawnEggItem((EntityType<? extends MobEntity>) entityType, this.primaryColor, this.secondaryColor,
                            (new Settings())),
                    new Identifier(IdentifierVoid.getNamespace(), String.format("%s_spawn_egg", IdentifierVoid.getPath())));

            com.KafuuChino0722.coreextensions.core.registrylib.Registry.addToItemGroup(ItemGroups.SPAWN_EGGS,spawn_egg);
        }
