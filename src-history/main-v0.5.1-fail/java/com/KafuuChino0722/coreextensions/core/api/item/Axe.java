...
public class Axe {
    ...
    public static void register(String name, String namespace, String id, Map<String, Object> itemData, Map<String, Object> properties, String level, boolean generate){
        ...
        ToolItem Axe = registerAxe(namespace, id, (float) attackDamage, (float) attackSpeed, durability, (ToolMaterials) material, rarityType, tooltipMsg);
        ...
        if(FabricLoader.getInstance().isModLoaded("polymc")) {
            ...
            Item vanillaItem = IdentifierManager.getItem((String) polyinfo.getOrDefault("vanilla","minecraft:iron_axe"));
            PolyMcLoader.loadTool.tool(Axe,vanillaItem);
        }
    ...
    public static ToolItem registerAxe(String namespace, String id, float attackDamage, float attackSpeed, int durability, ToolMaterials material, Rarity rarityType, String tooltipMsg) {
        return Registry.register(Registries.ITEM, new Identifier(namespace, id),
                new CustomAxeItem(material, (float) attackDamage, attackSpeed, new FabricItemSettings().maxDamage(durability), tooltipMsg));
    }
}
