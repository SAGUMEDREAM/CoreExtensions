...
public class SoulSand {
    ...
    public static void register(String name, String namespace, String id, int maxCount, Map<String, Object> blockData, Map<String, Object> properties, Boolean dropsNothing, BlockSoundGroup customSound ,double hardness, double resistance, boolean generate){
        FabricBlockSettings blockSettings = FabricBlockSettings.copyOf(Blocks.SAND)
                ...
                .sounds(BlockSoundGroup.WOOL).allowsSpawning(Blocks::always).solidBlock(Blocks::always).blockVision(Blocks::always).suffocates(Blocks::always);
        ...
        Block block = new SoulSandBlock(blockSettings);
        ...
        RuntimeResourcePack packs_tag = RuntimeResourcePack.create(new Identifier(namespace, id+"_tagsoulsand"));

        RRPCallback.BEFORE_VANILLA.register(b -> {
            packs_tag.clearResources();
            packs_tag.addTag(IdentifiedTagBuilder.createBlock(BlockTags.SOUL_SPEED_BLOCKS).add(new Identifier(namespace,id)));
            packs_tag.addTag(IdentifiedTagBuilder.createBlock(BlockTags.SOUL_FIRE_BASE_BLOCKS).add(new Identifier(namespace,id)));
            packs_tag.addTag(IdentifiedTagBuilder.createBlock(BlockTags.WITHER_SUMMON_BASE_BLOCKS).add(new Identifier(namespace,id)));
            b.add(packs_tag);
        });
