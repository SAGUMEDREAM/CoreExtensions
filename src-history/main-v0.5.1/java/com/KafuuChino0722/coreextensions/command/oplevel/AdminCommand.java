...
public class AdminCommand {
    ...
    public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
        dispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("op-admin").requires(source -> source.hasPermissionLevel(4))).then(CommandManager.argument("targets", GameProfileArgumentType.gameProfile()).suggests((context, builder) -> {
        ...
        }).executes(context -> AdminCommand.op((ServerCommandSource)context.getSource(), GameProfileArgumentType.getProfileArgument(context, "targets")))));
    }
    ...
    private static int op(ServerCommandSource source, Collection<GameProfile> targets) throws CommandSyntaxException {
        ...
        for (GameProfile gameProfile : targets) {
            ...
            new OperatorEntry(gameProfile, CommandPermissions.ADMIN, true);
            ++i;
