...
public class Pickaxe {
    ...
    public static void register(String name, String namespace, String id, Map<String, Object> itemData, Map<String, Object> properties , String level, boolean generate){
        ...
        ToolItem Pickaxe = new CustomPickaxeItem(material, (int) attackDamage, (float) attackSpeed, new FabricItemSettings().maxDamage(durability).rarity(rarityType), tooltipMsg);
        try {
            registerItem(namespace, id, Pickaxe);
        } catch (Exception e) {
            setRegistry.set(namespace,id,Pickaxe);
        }
        ...
        if(FabricLoader.getInstance().isModLoaded("polymc")) {
            ...
            Item vanillaItem = IdentifierManager.getItem((String) polyinfo.getOrDefault("vanilla","minecraft:iron_pickaxe"));
            PolyMcLoader.loadTool.tool(Pickaxe,vanillaItem);
        }
    ...
    public static ToolItem registerPickaxe(String namespace, String id, int attackDamage, float attackSpeed, int durability, ToolMaterials material, Rarity rarityType,String tooltipMsg) {
        return Registry.register(Registries.ITEM, new Identifier(namespace, id),
                new CustomPickaxeItem(material, attackDamage, attackSpeed, new FabricItemSettings().maxDamage(durability).rarity(rarityType), tooltipMsg));
    }
}
