...
public class Shield {
    ...
    public static void register(String name, String namespace, String id, Map<String, Object> itemData, Map<String, Object> properties, boolean generate){
        ...
        Item Shield = new FabricShieldItem(new FabricItemSettings().maxDamage(durability).rarity(rarityType), 10, 13, repairMaterialItem);
        try {
            registerItem(namespace, id, Shield);
        } catch (Exception e) {
            setRegistry.set(namespace, id, Shield);
        }
        ...
        if(FabricLoader.getInstance().isModLoaded("polymc")) {
            ...
            PolyMcLoader.loadTool.shield(Shield,vanillaItem);
        }
