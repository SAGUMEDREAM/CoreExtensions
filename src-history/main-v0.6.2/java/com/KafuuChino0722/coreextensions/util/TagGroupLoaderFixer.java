...
public class TagGroupLoaderFixer {
    ...
    public static boolean disableIngameError = Boolean.getBoolean("disable_error_output");
    ...
    public static void init(Path configPath) {
        ...
        try {
            if (configFile.createNewFile()) {
                LOGGER.info("Unable to find needed config file, will attempt to create such.");
            ...
            LOGGER.error("Unable to create the needed config file, using default values!", exception);
        } catch (JsonSyntaxException exception) {
            LOGGER.error("Unable to read the needed config file, using default values!", exception);
        }
    ...
    public static void sendMessage(PlayerEntity entity){
        ...
        entity.sendMessage(
                Text.literal("It seems that some tags are a bit cooked. Look at the Logs for more details on broken functions. Click me for more info about this feature.")
                        .styled(style -> {
