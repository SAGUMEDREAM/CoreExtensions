...
public class ActionSetFeed implements ActionInterface {
    public static void run(World world, LivingEntity user, Hand hand, Map<String, Object> Key) {
        Map<String, Object> HKey = (Map<String, Object>) Key.getOrDefault("setFeed", null);
        if(HKey!=null) {
            ...
            if (HKey != null && !player.getAbilities().creativeMode) {
                ...
                hungerM.setFoodLevel((int) hunger);
                hungerM.setSaturationLevel((float) saturationModifier);
            }
        }
    }
}
