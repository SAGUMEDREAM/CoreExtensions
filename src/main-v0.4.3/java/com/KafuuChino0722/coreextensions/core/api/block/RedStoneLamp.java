...
public class RedStoneLamp {
    public static void register(String name, String namespace, String id, int maxCount, Map<String, Object> blockData, Map<String, Object> properties, Boolean dropsNothing, BlockSoundGroup customSound ,double hardness, double resistance, boolean generate){
        FabricBlockSettings blockSettings = FabricBlockSettings.copyOf(Blocks.REDSTONE_LAMP)
                .strength((float) hardness, (float) resistance)
                .sounds(customSound).allowsSpawning(Blocks::always)
                .luminance(Blocks.createLightLevelFromLitBlockState((int) properties.getOrDefault("lightLevel", 0)));
        ...
        Block block = new RedstoneLampBlock(blockSettings);
