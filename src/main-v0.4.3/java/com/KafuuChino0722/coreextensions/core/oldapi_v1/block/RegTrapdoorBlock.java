package com.KafuuChino0722.coreextensions.core.oldapi_v1.block;
...
import static com.KafuuChino0722.coreextensions.Main.vanillaPacks;

public class RegTrapdoorBlock {
    ...
    public static void load() {
        ...
        try {
            Map<String, Map<String, Object>> blocksData = yaml.load(new FileReader(FILE + "blockTrapdoor.yml"));
                ...
                for (Map.Entry<String, Object> entry : blocks.entrySet()) {
                    if (entry.getValue() instanceof Map) {
                        ...
                        RRPCallback.BEFORE_VANILLA.register(b -> {
                            ...
                            vanillaPacks.addLang(new Identifier(namespace, "en_us"), LanguageProvider.create()
                                    .add("block." + namespace + "." + id, lang_us));
                            vanillaPacks.addLang(new Identifier(namespace, "zh_cn"), LanguageProvider.create()
            ...
            if (blocksData != null && blocksData.containsKey("ironDoors")) {
                Map<String, Object> blocks = blocksData.get("ironDoors");
                ...
                for (Map.Entry<String, Object> entry : blocks.entrySet()) {
                    if (entry.getValue() instanceof Map) {
                        ...
                        // 读取更多属性并设置到 blockSettings 中
                        // blockSettings = blockSettings.someProperty((SomeType) blockData.getOrDefault("propertyName", defaultValue));
                        Block block = new TrapdoorBlock(blockSettings, BlockSetType.IRON);
                        registerBlock(namespace, id, block);
                        registerBlockItem(namespace, id, block);
                        setupRenderLayer(block, blockData); // 设置渲染层
                        Main.LOGGER.info("Block " + name + "<->" + namespace + ":" + id + " registered!");
